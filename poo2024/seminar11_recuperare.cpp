...
public:
	...
	PersonalMedical(string nume, float salariuBaza) {
		this->nume = nume;
...
public:
	Medic() :PersonalMedical() {
		this->sporStudii = 10;
		this->specializare = "Stomatologie";
		/*this->specializare = new char[strlen("Stomatologie")+1];
		strcpy_s(this->specializare, strlen("Stomatologie") + 1, "Stomatologie");*/
	}

	Medic(string nume, float salariuBaza, float sporStudii, string specializare) :PersonalMedical(nume, salariuBaza) {
	...
	/*Medic operator=(const Medic& m) {
		if (&m != this) {
			...
			this->specializare = new char[strlen(m.specializare) + 1];
			strcpy(this->specializare, strlen(m.specializare) + 1, m.specializare);
			this->sporStudii = m.sporStudii;
		}
		return *this;
	}*/

	float getVenitTotal() { //suprascriere
...
int main() {
	...
	pointeri[4] = new Anestezist("Gheorghe", 1000); //reference type
