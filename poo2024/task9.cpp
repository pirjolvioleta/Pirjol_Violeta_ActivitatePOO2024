...
protected:
	string denumireInstrument;
public:
	InstrumentDeScris() {
		this->denumireInstrument = "instrument";
	}

	InstrumentDeScris(string denumireInstrument) {
		this->denumireInstrument = denumireInstrument;
	}
	...
	virtual string getDenumireInstrument() = 0;
};

class Stilou:public InstrumentDeScris { 
private:
...
public:
	Stilou() : InstrumentDeScris(), id(++numarStilouri) {
		this->culoareStilou = "Roz";
		this->lungime = 15;
		this->producator = NULL;
	}

	Stilou(string denumireInstrument, string culoareStilou, int lungime, const char* producator, int id) : InstrumentDeScris(denumireInstrument), id(id) {
	...
	Stilou(string denumireInstrument, string culoareStilou, const char* producator, int id) : InstrumentDeScris(denumireInstrument), id(id) {
		this->culoareStilou = culoareStilou;
		this->producator = new char[strlen(producator) + 1];
		strcpy_s(this->producator, strlen(producator) + 1, producator);
	}
	...
	Stilou(const Stilou& s) :InstrumentDeScris(), id(s.id) {
	...
	string getDenumireInstrument() {
		return this->denumireInstrument;
	}
...
class Highlighter: public InstrumentDeScris {
private:
	string culoareH;
	int greutate;
	char* brand;
...
public:
	Highlighter() :InstrumentDeScris(), cod(++numarHighlightere) {
		this->culoareH = "Roz pastel";
		this->greutate = 12;
		this->brand = NULL;
	}

	Highlighter(string culoareH, int greutate, const char* brand, int cod, string denumireInstrument) :InstrumentDeScris(denumireInstrument), cod(cod) {
	...
	/*Highlighter(string culoareH, const char* brand, int cod, string denumireInstrument) : InstrumentDeScris(denumireInstrument), cod(cod) {
		this->culoareH = culoareH;
		this->brand = new char[strlen(brand) + 1];
		strcpy_s(this->brand, strlen(brand) + 1, brand);
	}*/
	...
	Highlighter(const Highlighter& h) :InstrumentDeScris(h), cod(h.cod) {
...
public:
	...
	Birotica(string denumireProdus) {
		this->denumireProdus = denumireProdus;
	}

	virtual string getDenumireProdus() = 0;
};

class Caiet:public Birotica {
private:
	int nrPagini;
	float pret;
	char* tipCaiet;
...
public:
	Caiet() :Birotica(), codC(++numarCaiete) {
		this->nrPagini = 100;
		this->pret = 8;
		this->tipCaiet = NULL;
	}

	Caiet(int nrPagini, float pret, const char* tipCaiet, int codC, string denumireProdus) : Birotica(denumireProdus), codC(codC) {
	...
	/*Caiet(int nrPagini, float pret, int codC, string denumireProdus) :Birotica(denumireProdus), codC(codC) {
		this->nrPagini = nrPagini;
		this->pret = pret;
	}*/
	...
	Caiet(const Caiet& c) : Birotica(c), codC(c.codC) {
...
public:
	Magazin() {
		this->nume = NULL;
		this->stilou = new Stilou("stilou", "Albastru", "Stabilo", 12);
		this->nrCaiete = 0;
		this->caiete = NULL;
	}
...
void afisareVector(InstrumentDeScris** vector, int numar) {
	for (int i = 0; i < numar; i++) {
		cout << "Instrumentul " << (i + 1) << " este: " << vector[i]->getDenumireInstrument() << endl;
	}
}
...
int main() {
	//-----------task9----------
	InstrumentDeScris* instrument = new Stilou("stilou", "Rosu", 13, "Daco", 12);
	InstrumentDeScris** pointeri = new InstrumentDeScris * [10];
	pointeri[0] = new Stilou("stilou personalizat", "Verde", "Pelikan", 24);
	pointeri[1] = new Stilou("stilou cu penita", "Alb", "Stabilo", 13);
	pointeri[2] = new Highlighter("Galben", 15, "Daco", 34, "highlighter neon");
	pointeri[3] = new Highlighter("Roz", 21, "Daco", 10, "highlighter cu varf fin");
	pointeri[4] = new Highlighter("Albastru", 20, "Bic", 11, "highlighter cu varf tip pensula");
	pointeri[5] = new Stilou("stilou cu varf de pix", "Negru", "Schneider", 13);
	pointeri[6] = new Stilou("stilou scolar", "Multicolor", "Schneider", 31);
	pointeri[7] = new Highlighter("Lila", 19, "Stabilo", 12, "highlighter pastel");
	pointeri[8] = new Highlighter("Roz pastel", 18, "Stabilo", 15, "highlighter pastel");
	pointeri[9] = new Stilou("stilou metalic", "Magenta", "Herlitz", 29);

	afisareVector(pointeri, 10);

	Birotica* b = new Caiet(140, 20, "A4", 28, "caiet cu spirala");
	cout <<"Denumirea produsului: "<< b->getDenumireProdus() << endl;
